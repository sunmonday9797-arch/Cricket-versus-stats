...
if 'welcome_shown' not in st.session_state:
    st.success("🏏 This website is purely for members of the Gully Gang Cricket Club, developed by gang leader Rahul Raj with pure love for this game. Enjoy!")
    st.balloons()
...
def calculate_smart_rankings(df_all):
    ...
    # Base Profiles
    bat_profile = df_calc.groupby('Batsman')[['Runs', 'Balls', 'Wickets']].sum().reset_index()
    ...
    bwl_profile = df_calc.groupby('Bowler')[['Runs', 'Balls', 'Wickets']].sum().reset_index()
    bwl_profile['BwlAvg'] = np.where(bwl_profile['Wickets'] > 0, bwl_profile['Runs'] / bwl_profile['Wickets'], bwl_profile['Runs'] * 2.0)
    bwl_profile['Econ'] = np.where(bwl_profile['Balls'] > 0, (bwl_profile['Runs'] / bwl_profile['Balls']) * 6, 0)
    
    # True League Global Averages
    total_league_runs = df_calc['Runs'].sum()
    total_league_balls = df_calc['Balls'].sum()
    total_league_wickets = df_calc['Wickets'].sum()
    
    l_bat_avg = total_league_runs / total_league_wickets if total_league_wickets > 0 else 1
    l_sr = (total_league_runs / total_league_balls * 100) if total_league_balls > 0 else 1
    l_bwl_avg = total_league_runs / total_league_wickets if total_league_wickets > 0 else 1
    l_econ = (total_league_runs / total_league_balls * 6) if total_league_balls > 0 else 1
    
    # Compute Opposition Quality Index (OQI)
    ...
    bwl_profile['OQI'] = np.where((bwl_profile['BwlAvg'] > 0) & (bwl_profile['Econ'] > 0), 
                                   (l_bat_avg / bwl_profile['BwlAvg']) * (l_econ / bwl_profile['Econ']), 0.1)
    ...
    # --- SBR Core DataFrame Processing (BULLETPROOF METHOD) ---
    sbr_df = df_calc.groupby('Batsman')[['Balls', 'Weighted_Runs']].sum().reset_index()
    sbr_df.rename(columns={'Balls': 'Total_Balls'}, inplace=True)
    
    sbr_df = sbr_df.merge(bat_profile[['Batsman', 'BatAvg']], on='Batsman')
    sbr_df['SBR_Raw'] = np.where(sbr_df['Total_Balls'] > 0, (sbr_df['Weighted_Runs'] / sbr_df['Total_Balls']) * np.sqrt(sbr_df['BatAvg']), 0)
    
    max_sbr = sbr_df['SBR_Raw'].max() if not sbr_df.empty else 1
    sbr_df['SBR'] = np.where(max_sbr > 0, (sbr_df['SBR_Raw'] / max_sbr) * 100, 0)
    sbr_df = sbr_df.round({'SBR': 1, 'Weighted_Runs': 1, 'BatAvg': 2})
    sbr_df = sbr_df[['Batsman', 'SBR', 'Weighted_Runs', 'BatAvg']].sort_values(by='SBR', ascending=False).reset_index(drop=True)

    # --- SBO Core DataFrame Processing (BULLETPROOF METHOD) ---
    sbo_df = df_calc.groupby('Bowler')[['Wickets', 'Balls', 'Weighted_Wickets']].sum().reset_index()
    sbo_df.rename(columns={'Wickets': 'Wickets_Taken', 'Balls': 'Balls_Bowled'}, inplace=True)
    
    sbo_df = sbo_df.merge(bwl_profile[['Bowler', 'Econ']], on='Bowler')
    sbo_df['SBO_Raw'] = np.where((sbo_df['Balls_Bowled'] > 0) & (sbo_df['Econ'] > 0),
                                  (sbo_df['Weighted_Wickets'] / sbo_df['Balls_Bowled'] * 6) * (l_econ / sbo_df['Econ']) * np.sqrt(sbo_df['Wickets_Taken']), 0)
    
    max_sbo = sbo_df['SBO_Raw'].max() if not sbo_df.empty else 1
    sbo_df['SBO'] = np.where(max_sbo > 0, (sbo_df['SBO_Raw'] / max_sbo) * 100, 0)
    sbo_df = sbo_df.round({'SBO': 1, 'Weighted_Wickets': 1, 'Econ': 2})
    sbo_df = sbo_df[['Bowler', 'SBO', 'Weighted_Wickets', 'Wickets_Taken', 'Econ']].sort_values(by='SBO', ascending=False).reset_index(drop=True)

    # --- MVP Core DataFrame Processing ---
    mvp_df = pd.merge(sbr_df.rename(columns={'Batsman': 'Player'}), sbo_df.rename(columns={'Bowler': 'Player'}), on='Player', how='outer').fillna(0)
    mvp_df['Total_Smart_Points'] = (mvp_df['SBR'] + mvp_df['SBO']).round(1)
...
with tab1:
    ...
    # Extract unique player names and dates for the dropdowns
    all_players = []
    all_dates = []
    if not df.empty:
        all_players = sorted(list(set(df['Batsman'].dropna().unique()) | set(df['Bowler'].dropna().unique())))
        if 'Date' in df.columns:
            all_dates = sorted(list(df['Date'].dropna().unique()))
            
    player_options = [""] + all_players 
    date_options = [""] + all_dates
    
    col1, col2, col3, col4 = st.columns(4)
    p1 = col1.selectbox("Player 1:", options=player_options, key="p1_input")
    p2 = col2.selectbox("Player 2 (VS):", options=player_options, key="p2_input")
    date_filter = col3.selectbox("Date:", options=date_options, key="date_input")
    match_filter = col4.text_input("Match No:", key="match_input")

    st.markdown("### ⚙️ View Options")
    ctrl1, ctrl2, ctrl3 = st.columns(3)
    
    # Info (i) tooltip applied to Mode selection
    view_mode = ctrl1.radio(
        "Mode:", 
        ["All", "Batting", "Bowling"], 
        horizontal=True,
        help="This mode applies to Player 1 (the first search box). Example: Selecting 'Batting' filters to see only when Player 1 is batting."
    )
    ...
    if not filtered_df.empty:
        ...
        # Updated to exact match for dropdown integration
        if date_filter and 'Date' in filtered_df.columns:
            filtered_df = filtered_df[filtered_df['Date'].astype(str) == str(date_filter)]

        if p1:
            m1 = filtered_df['Batsman'] == p1
            v1 = filtered_df['Bowler'] == p1
        ...
        if p2:
            m2 = filtered_df['Batsman'] == p2
            v2 = filtered_df['Bowler'] == p2
    ...
    if filtered_df.empty:
        ...
        # Hide the raw cumulative tracking columns from the final clean display
        hide_cols = ['Cum_Runs', 'Cum_Balls', 'Cum_Wickets', 'is_bat']
        display_cols = [c for c in filtered_df.columns if c not in hide_cols]
        display_df = filtered_df[display_cols]
        
        # Lock Batsman and Bowler as the static index to guarantee sticky columns
        if 'Batsman' in display_df.columns and 'Bowler' in display_df.columns:
            display_df = display_df.set_index(['Batsman', 'Bowler'])
