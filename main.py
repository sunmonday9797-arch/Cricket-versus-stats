...
# --- WELCOME MESSAGE (Runs only once per session) ---
...
def calculate_smart_rankings(df_all):
    ...
    # 4. Vectorized Weighted Event Calculations
    ...
    # 5. Build Smart Batting Rankings
    sbr_df = df_calc.groupby('Batsman').agg(
        Total_Balls=('Balls', 'sum'),
        Weighted_Runs=('Weighted_Runs', 'sum')
    ).reset_index()
    sbr_df = sbr_df.merge(bat_profile[['Batsman', 'BatAvg']], on='Batsman')
    sbr_df['SBR'] = np.where(sbr_df['Total_Balls'] > 0, 
                             (sbr_df['Weighted_Runs'] / sbr_df['Total_Balls']) * np.sqrt(sbr_df['BatAvg']), 0)
    sbr_df['SBR'] = sbr_df['SBR'].round(2)
    sbr_df['Weighted_Runs'] = sbr_df['Weighted_Runs'].round(2)
    sbr_df = sbr_df[['Batsman', 'SBR', 'Weighted_Runs']].sort_values(by='SBR', ascending=False).reset_index(drop=True)
    sbr_df.index += 1 

    # 6. Build Smart Bowling Rankings
    if 'Match No' in df_calc.columns:
    ...
    sbo_df['SBO'] = sbo_df['SBO'].round(2)
    sbo_df['Weighted_Wickets'] = sbo_df['Weighted_Wickets'].round(2)
    sbo_df = sbo_df[['Bowler', 'SBO', 'Weighted_Wickets']].sort_values(by='SBO', ascending=False).reset_index(drop=True)
    sbo_df.index += 1

    # 7. Build MVP Rankings
    mvp_df = pd.merge(
        sbr_df.rename(columns={'Batsman': 'Player'}), 
        sbo_df.rename(columns={'Bowler': 'Player'}), 
        on='Player', how='outer'
    ).fillna(0)
