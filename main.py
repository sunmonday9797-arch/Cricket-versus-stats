...
def calculate_smart_rankings(df_all):
    ...
    # --- UPDATED: TRUE LEAGUE GLOBAL AVERAGES ---
    ...
    # Normalize SBR to a clean 0-100 scale
    max_sbr = sbr_df['SBR'].max() if not sbr_df.empty else 1
    sbr_df['SBR'] = np.where(max_sbr > 0, (sbr_df['SBR'] / max_sbr) * 100, 0)
    ...
    # SBO Rankings (Updated Rate, Economy & Volume Constraints)
    bowl_counts = df_calc.groupby('Bowler').agg(Wickets_Taken=('Wickets', 'sum'), Balls_Bowled=('Balls', 'sum'), Weighted_Wickets=('Weighted_Wickets', 'sum')).reset_index()
    bowl_counts = bowl_counts.merge(bwl_profile[['Bowler', 'Econ']], on='Bowler')
    
    # SBO Core Calculation: (Weighted Wickets / Balls * 6) * (League Eco / Eco) * sqrt(Wickets)
    bowl_counts['SBO_Raw'] = np.where((bowl_counts['Balls_Bowled'] > 0) & (bowl_counts['Econ'] > 0),
                                      (bowl_counts['Weighted_Wickets'] / bowl_counts['Balls_Bowled'] * 6) * (l_econ / bowl_counts['Econ']) * np.sqrt(bowl_counts['Wickets_Taken']), 0)
    
    # Normalize SBO to a clean 0-100 scale
    max_sbo = bowl_counts['SBO_Raw'].max() if not bowl_counts.empty else 1
    bowl_counts['SBO'] = np.where(max_sbo > 0, (bowl_counts['SBO_Raw'] / max_sbo) * 100, 0)
    
    sbo_df = bowl_counts.round({'SBO': 1, 'Weighted_Wickets': 1, 'Econ': 2})
    sbo_df = sbo_df[['Bowler', 'SBO', 'Weighted_Wickets', 'Wickets_Taken', 'Econ']].sort_values(by='SBO', ascending=False).reset_index(drop=True)
    sbo_df.index += 1

    # MVP Rankings (Combined Balance Matrix)
